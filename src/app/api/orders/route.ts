...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const line of lines) {
      const savedFile = await saveModelFile(line.file);
      const uploadedFile = await prisma.uploadedFile.create({ data: savedFile });
      savedLines.push({ uploadedFileId: uploadedFile.id, quantity: line.quantity });
    }
    ...
    const order = await prisma.order.create({
      data: {
        ...
        items: {
          create: savedLines.map((line, sortOrder) => ({
